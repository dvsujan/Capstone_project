...
const Product = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:12150/api/Product?id=${productId}`)
      .then((response) => { 
    ...
    axios
      .get(`http://localhost:12150/api/Product/Options?id=${productId}`)
      .then((response) => {
  ...
  return (
    ...
    <div className="product-bottom-bar"> 
        <div className="store-data"> 
            {userStore ? <a href="/store">{userStore.address}</a> : <a href="/store">Select a store</a>}
        </div> 
        <div className="product-borrom-right"> 
            <div>
                <div>
                Quantity:   
                <button className="quant-btn" onClick={()=>{ 
                    if(quantity > 1){
